...
Construct the full JSON payload string in your own context (not via shell heredoc or inline script). Choose a unique temp filename using the spec code range (e.g. `tmp-payload-US-001-US-015.json`). Write the file to `.archetipo/` using your file-writing tool. Then invoke `archetipo spec add --file <path>`. After the CLI exits, delete the temp file.
